...
@section('content')
<div class="wrapper">
<h2 class="text-center mb-5">Hand Foot Mouth Disease (HFMD) Statistics</h2>
    ...
    <div class="request-wrapper mb-5">
        <div class="row mb-5">
            ...
            <div class="col-sm-12 col-md-6 request-container text-center px-5">
                <h4><b>Select the area and year as below</b></h4>
                ...
                <form method="post" action="{{ route('dashboard.getDistrictDetails') }}">
                    ...
                    <div class="form-row text-left mt-3 flex-column">
                        ...
                        <button type="submit" class="btn btn-warning w-50 mt-5 mx-auto">Next <i class="fas fa-angle-right"></i></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="py-5">
        <div class="title vertical-center p-0">
            <h2>Malaysia State</h2>
        </div>
        <h6 class="text-center mb-5 pt-1">Click view more to visualize the HFMD yearly statistics</h6>
        <div class="row">
            @foreach($states as $state)
    ...
    <div class="row">
        ...
        <div class="col-xl-7 col-sm-12 case-box overview-map-wrapper" style="padding: 3rem 0.5rem;">
            <div class="subtitle">
                <p class="mb-0">Case Distribution in Malaysia</p>
                <div>
                    <label for="map-year" class="mb-0">at Year: </label>
                    <select name="map-year" id="map-year" onchange="changeYear()">
                        @foreach($years as $year)
                        <option value="{{ $year }}">{{ $year }}</option>
                        @endforeach
                    </select><br>
                    <small class="text-secondary">(Click to change year)</small>
                </div>
            </div>
    ...
    <div class="row">
            ...
            <div class="case-box">
                <div class="swiper-container">
                    <div class="swiper-wrapper">
                        @foreach($summary as $count => $sumData)
                        <div class="swiper-slide">
                            @foreach($sumData as $sum)
                            <div class="row">
                                <div class="col vertical-center h5 mb-0">{{ $sum['year'] }}</div>
                                <div class="col total-case">
                                    <div class="title-case">{{ number_format($sum['total']) }}</div>
                                    <div class="subtitle-case">Total</div>
                                </div>
                                <div class="col infected-case">
                                    <div class="title-case">{{ number_format($sum['infected']) }}</div>
                                    <div class="subtitle-case">Infected</div>
                                </div>
                                <div class="col death-case">
                                    <div class="title-case">{{ number_format($sum['death']) }}</div>
                                    <div class="subtitle-case">Death</div>
                                </div>
                            </div>
                            <hr>
        ...
        <div class="col-xl-6 col-sm-12">
            <div class="col-md-12 case-box" style="min-height: 470px;">
                ...
                <div class="subtitle">Age Group Distribution across Years</div>
                <div style="font-size:14px" class="text-right text-secondary text-small">* Click age group to hide</div>
